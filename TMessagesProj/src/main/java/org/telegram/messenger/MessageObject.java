...
public class MessageObject {
    ...
    public boolean isVoiceTranscriptionOpen() {
        return isVoice() && messageOwner != null && messageOwner.voiceTranscriptionOpen && messageOwner.voiceTranscription != null && (messageOwner.voiceTranscriptionFinal || TranscribeButton.isTranscribing(this)) && (NicegramBillingHelper.INSTANCE.getUserHasNgPremiumSub() || UserConfig.getInstance(currentAccount).isPremium());
    }
...
//            }
    ...
    private String nicegramTranslation;
    private String originalMessage;

    public void toggleTranslation(TranslationChangedCallback callback) {
        if (nicegramTranslation == null) {
            ...
            NicegramTranslator.INSTANCE.translate(text, toLang, s -> {
                ...
                nicegramTranslation = s;
                if (isVoiceWithTranscription()) {
                    originalMessage = messageOwner.voiceTranscription;
                    setTranslationText(NicegramTranslator.INSTANCE.applyTranslationToMessage(originalMessage, s));
                } else {
                    originalMessage = text;
                    setTranslationText(NicegramTranslator.INSTANCE.applyTranslationToMessage(text, s));
                }
                forceUpdate = true;
                resetLayout();
                callback.changed();
            ...
            nicegramTranslation = null;
            setTranslationText(originalMessage);
            forceUpdate = true;
            resetLayout();
            callback.changed();
        }
    ...
    public boolean hasTranslation() {
        return nicegramTranslation != null;
    }

    private String getTextToTranslate() {
        if (isVoiceWithTranscription()) return getVoiceTranscription().toString();
        if (mediaExists) return caption == null ? null : caption.toString();
        else return messageText == null ? null : messageText.toString();
    }

    private void setTranslationText(String text) {
        if (isVoiceWithTranscription() && messageOwner != null) messageOwner.voiceTranscription = text;
        else if (mediaExists) caption = text;
        else messageText = text;
    }

    private boolean isVoiceWithTranscription() {
        return isVoice() && !TextUtils.isEmpty(getVoiceTranscription());
    }

    public interface TranslationChangedCallback {
        void changed();
        void error();
    }
