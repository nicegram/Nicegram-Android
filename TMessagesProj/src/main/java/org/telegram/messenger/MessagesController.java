...
public class MessagesController extends BaseController implements NotificationCenter.NotificationCenterDelegate {
    ...
    private SharedPreferences nicegramPreference;
    ...
    public static SharedPreferences getNicegramSettings(int account) {
        return getInstance(account).nicegramPreference;
    }
    ...
    public MessagesController(int num) {
        ...
        if (currentAccount == 0) {
            ...
            nicegramPreference = ApplicationLoader.applicationContext.getSharedPreferences("nicegram", Activity.MODE_PRIVATE);
        } else {
            ...
            nicegramPreference = ApplicationLoader.applicationContext.getSharedPreferences("nicegram" + currentAccount, Activity.MODE_PRIVATE);
        }
    ...
    public void performLogout(int type) {
        ...
        long currentAccountId = UserConfig.getInstance(currentAccount).clientUserId;
        if (NicegramDoubleBottom.INSTANCE.getLoggedToDbot()
                || accountCount <= 2
                || NicegramDoubleBottom.INSTANCE.isDbotAccountId(currentAccountId)) {
            NicegramDoubleBottom.INSTANCE.disableDbot(ApplicationLoader.applicationContext);
        }
        // endregion
                ...
                for (int a = 0, size = updatesOnMainThreadFinal.size(); a < size; a++) {
                    ...
                    if (baseUpdate instanceof TLRPC.TL_updatePrivacy) {
                        ...
                        if (!(update.pending && TextUtils.isEmpty(update.text))) {
                            if (update.pending || !TranscribeButton.finishTranscription(null, update.transcription_id, update.text, false)) {
    ...
    public static String getRestrictionReason(ArrayList<TLRPC.TL_restrictionReason> reasons) {
        if (reasons.isEmpty() || (NicegramNetwork.INSTANCE.getChatsUnblocked() && reasons.size() == 1 && reasons.get(0).reason.equals("porn"))) {
            return null;
        }
