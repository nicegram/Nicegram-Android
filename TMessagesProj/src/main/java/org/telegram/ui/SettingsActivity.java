...
public class SettingsActivity extends BaseFragment implements NotificationCenter.NotificationCenterDelegate, ImageUpdater.ImageUpdaterDelegate, MainTabsActivity.TabFragmentDelegate, FactorAnimator.Target {
    ...
    private static final int AI_ASSISTANT_ID = 1001,
            NG_PREMIUM_SETTING_ID = 1002,
            NG_SETTING_ID = 1003,
            NG_TELEGRAM_SESSION_ID = 1004;
...
//        fragmentView.addView(navigationBar, LayoutHelper.createFrame(LayoutHelper.MATCH_PARENT, 0, Gravity.BOTTOM | Gravity.FILL_HORIZONTAL));
    ...
    private void fillItems(ArrayList<UItem> items, UniversalAdapter adapter) {
        ...
        // region ng settings
        items.add(SettingCell.Factory.of(AI_ASSISTANT_ID, 0xFF55CA47, 0xFF27B434, R.drawable.ic_ng_ai_assistant, getString(R.string.NicegramAssistant_AiAssistant)));
        items.add(SettingCell.Factory.of(NG_PREMIUM_SETTING_ID, 0xFFF09F1B, 0xFFE18A11, R.drawable.ic_ng_premium_settings, getString(R.string.NicegramAssistant_Premium)));
        items.add(SettingCell.Factory.of(NG_SETTING_ID, 0xFF1BA4ED, 0xFF1488E1, R.drawable.ic_ng_settings, getString(R.string.NicegramAssistant_Nicegram)));
        items.add(SettingCell.Factory.of(NG_TELEGRAM_SESSION_ID, 0xFFFA0C8F, 0xFFFA0C8F, R.drawable.ic_telegram_session_settings_padding, getString(R.string.Nicegram_TelegramSession_Settings)));
    ...
    private void onClick(UItem item, View view, int position, float x, float y) {
        ...
        switch (item.id) {
            ...
            case AI_ASSISTANT_ID:
                presentFragment(new AiBotSettingsActivity());
                break;
            case NG_PREMIUM_SETTING_ID: {
                if (NicegramBillingHelper.INSTANCE.getUserHasNgPremiumSub(getContext())) {
                    presentFragment(new NicegramPremiumSettingsActivity());
                } else {
                    Activity activity = getParentActivity();
                    activity.startActivity(new Intent(activity, NicegramPremiumActivity.class));
                }
                break;
            }
            case NG_SETTING_ID:
                ...
                break;
            case NG_TELEGRAM_SESSION_ID:
                Activity activity = getParentActivity();
                if (activity != null) {
                    MainActivity.Companion.launchTelegramSession(activity, TelegramSessionSource.SETTINGS);
                }
                break;
        }
    }
