...
//        });
    ...
    private void setVoiceInputFieldVisible(boolean visible) {
        ...
        // richDraftActive keeps the field/emoji hidden in favour of richDraftPreview — don't
        // resurrect them here if a rich draft became active while the bar was up.
        final boolean showField = visible && !richDraftActive;
        if (messageEditText != null) {
            messageEditText.setVisibility(showField ? VISIBLE : GONE);
        }
        if (emojiButton != null) {
            emojiButton.setVisibility(showField ? VISIBLE : GONE);
        }
...
//        }
    ...
    private void updateRichDraftPreview() {
        ...
        if (richDraftActive) {
            ...
            richDraftPreview.setVisibility(View.GONE);
            // nicegram voice input: while our recording bar owns the input row, the field and emoji
            // button are deliberately GONE (see setVoiceInputFieldVisible). Replying with text in the
            // field saves a draft, which routes ChatActivity.applyDraftMaybe -> setRichDraftPreview
            // here and used to force both back VISIBLE on top of the bar. setVoiceInputFieldVisible(true)
            // restores them when the bar goes away.
            if (!voiceInputFieldHidden) {
                if (messageEditText != null) {
                    messageEditText.setVisibility(View.VISIBLE);
                }
...
//                }
        ...
        } else if (id == NotificationCenter.translationLanguageChanged) { //region ng translate input message
            setNewNaming();
        }
    }
...
//            }
    ...
    private void updateAttachLayoutParams() {
        ...
        if (attachLayout != null) {
            ...
            // nicegram voice input: never resurrect the attach row over the recording bar.
            attachLayout.setVisibility(attachLayout.getAlpha() > 0 && !voiceInputFieldHidden ? View.VISIBLE : View.GONE);
